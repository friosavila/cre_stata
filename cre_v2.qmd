...
Based on @wooldridge2019, the APE for a variable $x_{i,t}$ in a nonlinear model with unobserved heterogneity can be calculated as folows:
...
In other words, we when applying the CRE approach, the auxiliary terms $\bar x_i$ are treated as fixed, when calculating the APEs for all other variables in the model. On this point, @wooldridge2019 argues that standard errors for this expression can be difficult to obtain. However, Stata's `margins` command provides a convenient way to calculate APEs and their standard errors based on the delta method.
