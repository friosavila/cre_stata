...
save sim_v2/cre_probit_two, replace

parallel sim , reps(10000) seeds($seed): cre_sim, model(2)
save sim_v2/cre_fprobit_two, replace

parallel sim , reps(10000) seeds($seed): cre_sim, model(3)
save sim_v2/cre_tobit_two, replace

parallel sim , reps(10000) seeds($seed): cre_sim, model(4)
save sim_v2/cre_poisson_two, replace

use sim_v2/cre_probit_two, clear
...
use sim_v2/cre_fprobit_two, clear
two (kdensity  true_b_x1) (kdensity  nofe_b_x1) (kdensity  cre_b_x1 ) , ///
	legend(order(1 "Unfeasible" 2 "Pool Data no FE" 3 "CRE") col(3)) xtitle("Marginal Effect X1") ///
...
use sim_v2/cre_tobit_two, clear
two (kdensity  true_b_x1) (kdensity  nofe_b_x1) (kdensity  cre_b_x1 ) , ///
	legend(order(1 "Unfeasible" 2 "Pool Data no FE" 3 "CRE") col(3)) xtitle("Marginal Effect X1") ///
...
use sim_v2/cre_poisson_two, clear
two (kdensity  true_b_x1) (kdensity  nofe_b_x1) (kdensity  cre_b_x1 ) , ///
	legend(order(1 "Unfeasible" 2 "Pool Data no FE" 3 "CRE") col(3)) xtitle("Marginal Effect X1") ///
...
graph export sim_v2/fig1_two.png, replace width(1000)


esttab m1 m2 m3 m4 using sim_v2/table1_two.txt, cell(mean(fmt(3))) nonum mtitle(Probit FProbit Tobit Poisson) collab(none) ///
